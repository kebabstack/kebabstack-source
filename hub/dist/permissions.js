...
(() => {
  ...
  window.KebabPermissions = {
    create(root, api, {role = () => "", onActivity = () => {}} = {}) {
      ...
      root.innerHTML = `<div class="perm-heading"><div><span class="perm-eyebrow">Hub / Permissions</span><h1>Who can do what</h1><p class="lead">One role per person and app. Global Hub owners and admins inherit Admin everywhere. <a href="#/settings/teams">Manage the Finance team</a>.</p></div></div>
      ...
      const options = (selected, inherit=false) => `${inherit?'<option value="">Use inherited role</option>':""}${(current?.roles||[]).filter(r=>r.id!=="finance").map(r=>`<option value="${esc(r.id)}" ${r.id===selected?"selected":""}>${esc(r.name)}</option>`).join("")}`;
      ...
      function render() {
        ...
        <section class="card"><div class="perm-section-head"><div><h2>Default for employees</h2></div><select data-p="default" aria-label="Default employee role" ${owner?"":"disabled data-locked=true"}>${current.roles.filter(r=>!["admin","agent","finance"].includes(r.id)).map(r=>`<option value="${esc(r.id)}" ${draft.defaultRole===r.id?"selected":""}>${esc(r.name)}</option>`).join("")}</select></div><p data-p="default-description">${esc(selected?.can.join(" · ") || "No protected access to this app.")}</p></section>
        ...
        <details class="card" data-p="group-details"><summary>Group assignments <span class="kv">${draft.groups.length} configured</span></summary><p class="kv">The highest group role wins. Individual No access and Admin assignments take precedence. Finance membership supplements Employee access. Role-bearing manual groups are owner-managed in Hub People. IdP-managed groups follow their trusted SCIM source.</p><div class="perm-groups">${[...groups,...draft.groups.filter(x=>!groups.some(g=>g.id===x.id)).map(x=>({id:x.id,name:`Deleted group #${x.id} — remove assignment`}))].map(g=>{const grant=draft.groups.find(x=>x.id===g.id);return `<label>${esc(g.name)}<select data-group="${g.id}" aria-label="Role for group ${esc(g.name)}" ${owner?"":"disabled data-locked=true"}><option value="">No group assignment</option>${current.roles.filter(r=>r.id!=="none"&&r.id!=="finance").map(r=>`<option value="${r.id}" ${grant?.role===r.id?"selected":""}>${esc(r.name)}</option>`).join("")}</select></label>`;}).join("")||'<p class="kv">No Hub groups.</p>'}</div></details>
