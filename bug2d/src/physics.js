...
export function ascentAt() { return 0; }
...
export function launchVelocity(charge, degrees) {
  const power = 75 + 45 * charge + (charge >= .94 ? 10 : 0);
  const angle = clamp(degrees, 25, 60) * Math.PI / 180;
  return { speed: Math.cos(angle) * power, vy: Math.sin(angle) * power * .52 };
}
export function launch(run) {
  if (run.phase !== 'charging') return false;
  Object.assign(run, launchVelocity(run.charge, run.angle));
  run.phase = 'flying';
...
export function boost(run) {
  ...
  run.prompts--;
  run.speed = Math.min(150, run.speed + 40);
...
export function findTarget(run, objects = []) {
  ...
  for (const obj of [...objects, ghostTarget(run)].filter(Boolean)) {
    ...
    if (d < 3 || d > 105 || dx > 5 + d * .12 || dy > (obj.kind === 'ghost' ? 14 + d * .65 : 8 + d * .32)) continue;
...
function ghostAim(run, shot, ghost, speed) {
  // Lead the moving companion. Aiming at its old position misses a wide orbit.
  const travel = clamp(Math.hypot(ghost.d-shot.d, ghost.y-shot.y) / Math.max(100,speed-run.speed),0,.65);
  return { dx: 0, dy: ghost.y + run.vy * travel - shot.y, dd: ghost.d + run.speed * travel - shot.d };
}
export function fire(run, objects = []) {
  ...
  if (target?.kind === 'ghost') ({dx,dy,dd}=ghostAim(run,{d:run.d,y:height},target,speed));
  const length = Math.hypot(dx, dy, dd);
  run.shotsFired++; run.shotCooldown = SHOT_INTERVAL;
  run.weaponHeat = Math.min(1, run.weaponHeat + .24);
  if (run.weaponHeat >= .99) { run.overheated = true; run.overheats++; run.effects.push({kind:'overheat',label:'BLASTER OVERHEATED',sub:'Let it cool. Save a boost for the next threat.'}); }
...
export function stepProjectiles(run, dt, objects) {
  for (const shot of run.projectiles) {
    ...
    if (ghost && shot.targetId === ghost.id && ghost.d > shot.d) {
      ...
      const length = Math.hypot(dx, dy, dd), blend = 1 - Math.exp(-12 * dt);
...
export function stepRun(run, dt, _legacySteering = 0, objects = []) {
  if (!(dt > 0) || dt > 0.05 || !Number.isFinite(dt)) return;
  if (run.phase === 'charging') { run.chargeTime += dt; run.charge = (1 - Math.cos(run.chargeTime / .9 * Math.PI)) / 2; return; }
  ...
  // position. Wind cannot stretch this into a slow crawl.
  if (run.stillTime > 0) { run.burnTotal += burnRate(run) * dt; settle(run, dt); return; }
  const before = { x: run.x, y: run.y, d: run.d };
  // Automatic forward flight: only launch, boost and fire change the route.
  run.x = 0; run.vx = 0;
  ...
  run.d += run.speed * dt;
  run.y += run.vy * dt;
  if (run.y > 145) { run.y = 145; run.vy = Math.min(0, run.vy); }
  if (run.y <= 1.15) {
    run.y = 1.15;
...
export function makeObjects(day, chunk) {
  ...
  if (random() < .7) add('coffee', (random() - .5) * 40, 6 + random() * 22, start + 123, 3.3);
  add(special, (random() - .5) * 38, 6 + random() * 22, start + 176, 4.2);
  if (chunk > 1 && chunk < 20 && random() < .35) add(special, (random() - .5) * 44, 5, start + 112, 4);
  // Floor rescues are lucky finds, never a guaranteed conveyor belt.
  if (random() < .55) add('pad', (random() - .5) * 44, 1.5, start + 140, 4.3);
  if (chunk > 0) {
  ...
  // Keep the original coin IDs/forward positions for server validation.
  // Collapse depth to zero and retain one floor mine per row, plus airborne mines.
  const seen = new Set();
  const mines = makeMines(day, chunk).filter(m => { const k = `${m.d}:${m.airborne}`; if (seen.has(k)) return false; seen.add(k); return true; });
  return objects.concat(mines).map(o => ({...o, x: 0}));
}
