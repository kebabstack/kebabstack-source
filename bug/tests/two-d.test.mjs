...
import {createRun,beginCharge,launch,boost,stepRun,makeObjects,STEP,fire,VERSION,findTarget} from '../src/two-d/physics.js';
import {GameView,buildingLayout,viewScale} from '../src/two-d/scene.js';
import {stepGhost} from '../src/two-d/ghost.js';
...
test('automatic flight ignores old steering inputs and covers 130m in one second with a punchier first arc',()=>{
 const up=launched(),down=launched();assert.ok(up.speed>131);
 for(let i=0;i<120;i++){stepRun(up,STEP,1,[]);stepRun(down,STEP,-1,[]);}
 assert.equal(up.y,down.y);assert.equal(up.d,down.d);assert.ok(up.d>130);assert.equal(up.x,0);
 const speed=up.speed;boost(up);assert.ok(up.speed>=Math.min(150,speed+47));
 const arc=launched();while(!arc.bounces&&arc.elapsed<6)stepRun(arc,STEP,0,[]);assert.ok(arc.bounces>0&&arc.elapsed<5,'first arc must not float for six seconds');
});
test('Motoko strafes within the 2D plane and remains reachable with automatic aim',()=>{
 const r=launched();r.d=600;r.y=45;r.vy=0;let shots=0,sawTarget=false;
 for(let i=0;i<800&&r.ghost.kills===0;i++){
  if(findTarget(r,[])?.kind==='ghost'){sawTarget=true;if(i%35===0&&fire(r,[]))shots++;}
  stepRun(r,STEP,0,[]);assert.equal(r.ghost.x,0);
 }
 assert.ok(sawTarget);assert.equal(r.ghost.kills,1);assert.ok(shots<=8,'strafe must remain shootable');
});
test('every DFINITY window stays inside the façade above its street foundation',()=>{
 for(const scale of [2.8,3,4.2,5.3,6]){
...
test('Canvas2D renders every chapter with bounded resources across viewport sizes',()=>{
 ...
 globalThis.document={createElement:()=>({getContext:()=>ctx,setAttribute(){}})};globalThis.matchMedia=()=>({matches:false});globalThis.window={addEventListener(){}};
 for(const [width,height]of[[1440,900],[390,844],[844,390]]){
  ...
  v.reset(1,2);v.update(createRun(),0);assert.equal(v.particles.length,0);assert.equal(v.ctx.imageSmoothingEnabled,false);assert.equal(v.canvas.width,Math.round(width/2));
 }
 assert.ok(calls>1000);assert.equal(VERSION,readFileSync(new URL('../mops.toml',import.meta.url),'utf8').match(/^version\s*=\s*"([^"]+)"/m)[1]);
});

test('desktop scrolling has the original arcade pace without exceeding score speed limits',()=>{
 const r=launched(),pixelsPerSecond=r.speed*viewScale(1440,900);
 assert.ok(pixelsPerSecond>740 && pixelsPerSecond<900);
 assert.ok(r.speed<=150);assert.ok(r.speed*viewScale(390,844)>350);
});
