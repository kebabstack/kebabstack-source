...
export function launchVelocity(charge, degrees) {
  const power = 100 + 55 * charge + (charge >= .94 ? 12 : 0);
  const angle = clamp(degrees, 25, 60) * Math.PI / 180;
  return { speed: Math.cos(angle) * power, vy: Math.sin(angle) * power * .4 };
}
...
export function boost(run) {
  ...
  run.prompts--;
  run.speed = Math.min(150, run.speed + 48);
  run.vy = Math.max(run.vy, 10) + 22;
  run.boostTime = 0.65;
  run.stillTime = 0;
...
export function collect(run, obj) {
  ...
  if (!['cycle', 'pad', 'hazard', 'mine', 'updraft'].includes(obj.kind)) run.seals.add(obj.kind);
  if (obj.kind === 'cycle') {
    run.coins++; run.coinIds.push(obj.id); run.cycles += 10; run.combo++;
    flowCoin(run);
  } else if (obj.kind === 'updraft') {
    run.speed = Math.min(150, run.speed + 26 * power); run.vy = Math.max(run.vy, 27 * power); run.boostTime = .65;
    run.effects.push({kind: 'updraft', label: 'SOLAR CURRENT', sub: 'Free lift. Save your five prompts for the gaps.'});
...
export function stepRun(run, dt, _legacySteering = 0, objects = []) {
  ...
  if (run.phase === 'charging') { run.chargeTime += dt; run.charge = (1 - Math.cos(run.chargeTime / .75 * Math.PI)) / 2; return; }
  ...
  for (const obj of objects) {
    ...
    if (obj.kind === 'updraft' ? crossesCurrent(before, run, obj, 1.25) : obj.kind === 'mine' ? objectHit(before, run, obj, 1.25) : segmentDistanceSq(before, run, obj) <= reach ** 2) collect(run, obj);
  }
...
export function makeObjects(day, chunk) {
  ...
  const special = ['portal', 'coffee', 'compiler', 'canister', 'identity', 'oisy', 'fusion', 'portal', 'neuron', 'fusion'][zoneIndex(chunk * 200 + 160)];
  return courseObjects(day, chunk, random, special);
}
