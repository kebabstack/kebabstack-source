...
persistent actor Bug {
  ...
  // Public player ownership survives Hub expiry; it grants no company access.
  let arcadePilotKeys : Map.Map<Principal, Text> = Map.empty();
  include Arcade(arcadeProfiles, arcadeAliases, arcadeLinks, arcadeRuns, arcadeScores, arcadeCounters, arcadePilotKeys,
    func() : Text { hubId },
    ...
    func(tok : Text) : ?Text {
      // Legacy links were established by verified Hub tickets. Read only the
      // retained session before expiry pruning; never infer ownership from a name.
      switch (sessions.get(tok)) { case (?s) ?pidOf(s.email); case null null }
    },
  ...
  transient let BUILD_VERSION : Text = "0.17.5";
  ...
  public shared query func hub_manifest() : async Hub.Manifest {
    {
      name = "bug"; version = BUILD_VERSION; description = "Ship the Bug — 2D and 3D; shared pilots, separate scores, opt-in publication";
      needs = ["identity"];
  ...
  // Recover provable pre-0.17.0 associations before the first session-prune timer.
  do {
    for ((caller, _) in arcadeLinks.entries().toArray().values()) { ignore arcadeOwner(caller) };
  };
